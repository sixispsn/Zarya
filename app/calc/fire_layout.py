...
расчётным числом струй. Для типового прямоугольного помещения модуль использует
проверочную геометрическую модель досягаемости:
...
Это сохранённый инженерный алгоритм расстановки, а не ссылка на действующую
формулу СП: Изменением № 1 пункт 6.2.12 исключён. Нормативное соответствие
подтверждается отдельной проверкой покрытия по пп. 6.1.13 и 6.2.2.
...
def compute_spacing_L(
    ...
    """Проектный шаг между ПК: ``L = √(reach² − cross²)``.

    cross — поперечное плечо: ONE_SIDE → B (весь поперечник),
    TWO_OPPOSITE_SIDES → B/2.
    Формула служит оптимизатором начальной расстановки; итог проверяется по
    фактической досягаемости и кратности каждой контрольной точки.
    """
