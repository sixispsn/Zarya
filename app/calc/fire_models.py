...
class JetParams:
    ...
    valve_axis_height_m: принятая проектная высота оси клапана над полом.
        Изменением № 1 прежний п. 6.2.12 исключён, поэтому это не скрытый
        нормативный дефолт действующей редакции СП 10.
    """
    hose_length_m: float
...
class FireCabinetNormative:
    ...
    required_jets: сколько струй должно достигать каждой точки (1 или 2), а не
        общее число одновременно действующих ПК-с из таблицы 7.1/7.2.
