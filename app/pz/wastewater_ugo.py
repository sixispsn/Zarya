...
_DEFINITIONS: Tuple[UGOSymbolDefinition, ...] = (
    ...
    UGOSymbolDefinition(
        "pressure_break_loop",
        "петля гашения напора перед присоединением к самотечной сети",
        "СП 30.13330.2020, п. 18.32; проектное обозначение раскрывается в легенде",
        "project",
        305,
    ),
...
def _shape(kind: str) -> str:
    ...
    if kind == "pressure_break_loop":
        # СП 30.13330.2020, п. 18.32 требует петлю гашения напора, но не
        # задаёт для неё отдельного нормативного УГО. Поэтому это раскрытое
        # проектное обозначение фактической петли трубопровода.
        return (
            f'<path d="M-28,12 H-12 V-10 C-12,-24 12,-24 12,-10 '
            f'V12 H28" fill="none" stroke="{s}" stroke-width="{sw}"/>'
        )
    if kind == "junction":
        return (
