...
def _turn_service_check(
    ...
    return TurnServiceCheck(
        ...
        status="fail",
        note=(
            "нет подтверждённого пути троса: нужна доступная ревизия перед "
            "поворотом либо прочистка wye_45 на выходящем участке"
        ),
    )


def service_limit_m(system: str, nominal_dn: int, kind: str) -> Optional[float]:
    """Таблица 18.1 СП 30 для бытовых стоков и водостоков."""
