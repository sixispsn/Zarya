    ...
    *,
    ...
    ps.sp10_checks = [
        ...
        PumpComplianceCheck(
            clause="12.3",
            requirement=(
                "Резерв наиболее производительного агрегата; запрет автоматического "
                "отключения резервного пожарного насоса по перегрузке и сухому ходу"
            ),
            decision=(
                "Предусмотрен 1 рабочий и 1 однотипный резервный агрегат с АВР; "
                "для резервного насоса не предусматривается автоматическое защитное "
                "отключение по токовой/температурной перегрузке и сухому ходу"
            ),
            status="specified",
        ),
        PumpComplianceCheck(
        ...
        PumpComplianceCheck(
            clause="12.17",
            requirement="При расходе ВПВ более 10 л/с — подключение мобильной пожарной техники",
            decision=(
                "Предусмотреть не менее двух наружных патрубков DN 80; их количество "
                "должно пропускать расчётный расход"
                if q_lps > 10.0 else
                f"Расчётный расход {q_lps:g} л/с не превышает 10 л/с; условие п. 12.17 не наступило"
            ),
            status="specified",
        ),
        PumpComplianceCheck(
        ...
        PumpComplianceCheck(
            clause="12.23",
            requirement="Защита насосной станции от аварийного затопления",
            decision="Предусмотреть минимум один дренажный насос",
            status="specified",
        ),
        PumpComplianceCheck(
            clause="12.27",
            requirement="Количество входных всасывающих или входных напорных трубопроводов",
            decision=(
                "При количестве ПК менее 12 допускается один входной напорный трубопровод"
                if pk_total is not None and pk_total < 12 else
                "Предусмотреть не менее двух входных трубопроводов, каждый на полный расчётный расход"
                if pk_total is not None else
                "Количество ПК не передано; число входных трубопроводов требует проверки"
            ),
            status="specified" if pk_total is not None else "pending",
        ),
        PumpComplianceCheck(
            clause="12.29",
            requirement="Всасывающий трубопровод при работе без подпора",
            decision=(
                "Предусмотреть непрерывный подъём к насосу с уклоном не менее 0,005 "
                "и эксцентрические переходы; без нормирования уклона — только при "
                "явном удалении воздуха в наивысшей точке"
            ),
            status="specified",
        ),
        PumpComplianceCheck(
            ...
            decision=(
                "На напоре: манометр, обратный клапан и запорное устройство; на всасе: "
                "манометр и запорное устройство. Манометры — виброустойчивые, с "
                "возможностью гидрозаполнения"
            ),
            status="specified",
        ),
        PumpComplianceCheck(
            ...
            decision=(
                "Автоматический/дистанционный пуск после контроля давления; контроль "
                "давления у каждого агрегата и аварийного уровня воды на отметке "
                "фундаментов электроприводов"
            ),
            status="specified",
        ),
        PumpComplianceCheck(
