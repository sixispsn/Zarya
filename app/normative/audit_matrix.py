...
def build_ios2_gost_matrix(
    ...
    for rule_id, clauses, title, systems, required in (
        ...
        ("V-GOST-A07", ("5.2.1.5", "5.2.3.1–5.2.3.4"), "План наружной сети водоснабжения от точки подключения до ввода сформирован", ("В1", "В2", "Т3", "Т4"), ("План сетей водоснабжения",)),
    ):
        applicable = not (
        ...
        if not applicable:
            ...
            status, evidence = _artifact_status(artifacts, required)
        if rule_id == "V-GOST-A07" and status == "missing":
            evidence += (
                "; требуется опорный план с трассой от точки подключения до ввода, "
                "диаметрами, привязками и абсолютными отметками; "
                "принципиальная схема здания не заменяет этот план"
            )
        rows.append(make(
