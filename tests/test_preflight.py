...
def test_demo_passes_preflight_and_exposes_stage_r_boundary():
    ...
    assert not report.release_blockers
    assert report.normative_baseline.baseline_id == "ru-ios-2026-09-05-v1"
    assert any(
        row.rule.rule_id == "SP10-EDITION-ADOPTION-001"
        and row.status.value == "pass"
        and not row.release_blocking
...
def test_preflight_api_returns_structured_report_without_calculation():
    ...
    assert payload["counts"]["release_blocking"] == 0
